...
class DeviceControl(Base):
    ...
    user_id = Column(Integer, ForeignKey("User.id", ondelete='CASCADE', name='fk_device_control_user_id'), nullable=False)
    device_id = Column(String(100), nullable=False)
    device_name = Column(String(255), nullable=False)
    device_type = Column(SQLAEnum(DeviceType), nullable=False)
    status = Column(SQLAEnum(DeviceStatus), nullable=False, default=DeviceStatus.OFFLINE)
    ...
    control_logs = relationship("DeviceControlLog", back_populates="device", cascade="all, delete-orphan")
...
class DeviceControlLog(Base):
    ...
    id = Column(Integer, primary_key=True)
    device_id = Column(Integer, ForeignKey("device_controls.id", ondelete='CASCADE',name='fk_device_log_device_id'), nullable=False)
    user_id = Column(Integer, ForeignKey("User.id", ondelete='CASCADE',name='fk_device_log_user_id'), nullable=False)
