...
def backup_database(backup_path=None, custom_name=None):
    ...
    try:
        ...
        # Get backup filename
        backup_file = get_backup_filename(custom_name) if custom_name else get_backup_filename()
        final_backup_path = backup_path if backup_path else os.path.join(backup_dir, backup_file)
        ...
        process = subprocess.Popen(
            ...
            env=dict(os.environ, PGPASSWORD=db_password)  # Set password in environment
        )
        ...
        logger.info(f"Database backup completed successfully: {compressed_file}")
        return True
...
def list_backups():
    ...
    backups = sorted(backup_dir.glob('*.gz'), key=lambda x: x.stat().st_mtime, reverse=True)
    if not backups:
        print("No backups found.")
        return
...
def cleanup_old_backups(days=7):
    ...
    try:
        ...
        for backup in backup_dir.glob('*.gz'):
            file_age_days = (current_time - backup.stat().st_mtime) / (24 * 3600)
