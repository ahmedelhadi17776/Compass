...
class AIOrchestrator:
    ...
    async def process_request(self, user_input: str, user_id: int) -> Dict[str, Any]:
        ...
        # Step 7: Formulate the AI query using dynamic templates
        ...
        self.logger.debug(
            f"Generated prompt using template variant '{template_variant}' for domain '{target}'")

        # Step 8: Generate the AI response
        ai_response = await self.llm_service.generate_response(prompt)
        response_text = ai_response if isinstance(
            ai_response, str) else ai_response.get("text", "")
        confidence = 0.0 if isinstance(
            ai_response, str) else ai_response.get("confidence", 0.0)

        # Step 9: Store the response in cache (if enabled)
        if cache_key:
            cache_data = {
