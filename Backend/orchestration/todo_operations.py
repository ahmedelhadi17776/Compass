...
async def try_backend_urls(client_func, endpoint: str, **kwargs) -> Dict[str, Any]:
    ...
    logger.info(f"[CONNECTION] Trying to connect to endpoint {endpoint} with {len(GO_BACKEND_URLS)} URLs")
    ...
    for base_url in GO_BACKEND_URLS:
        ...
        try:
            async with httpx.AsyncClient(timeout=timeout) as client:
                logger.info(f"[CONNECTION] Sending {client_func.__name__} request to {full_url}")
                response = await client_func(client, full_url, **kwargs)
                logger.info(f"[CONNECTION] Received response from {full_url}: status={response.status_code}")
                response.raise_for_status()

                previous_url = GO_BACKEND_URL
                GO_BACKEND_URL = base_url
                logger.info(f"[CONNECTION] CONNECTION SUCCESS! {base_url} is working")
                logger.info(f"[CONNECTION] Updated primary backend URL from {previous_url} to {GO_BACKEND_URL}")

                try:
                    result = response.json()
                    logger.info(f"[CONNECTION] Successfully parsed JSON response from {base_url}")
                    return result
                except Exception as json_error:
                    logger.warning(f"[CONNECTION] Response not JSON: {str(json_error)}")
                    return {"status": "success", "message": response.text}
        except Exception as e:
            logger.warning(f"[CONNECTION] Failed to connect to {base_url}: {str(e)}")
            errors.append({"url": base_url, "error": str(e), "type": "connection_error"})

    error_msg = f"Failed to connect to any backend URL: {[e['url'] for e in errors]}"
    logger.error(f"[CONNECTION] ALL CONNECTION ATTEMPTS FAILED. Tried URLs: {GO_BACKEND_URLS}")
