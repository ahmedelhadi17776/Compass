...
class DataRequest(Base):
    ...
    user_id = Column(Integer, ForeignKey(
        "users.id", ondelete='CASCADE', name='fk_data_request_user_id'), nullable=False)
    request_type = Column(SQLAEnum(DataRequestType), nullable=False)
    status = Column(SQLAEnum(DataRequestStatus), nullable=False,
    ...
    archives = relationship(
        "DataArchive", back_populates="data_request", cascade="all, delete-orphan")
...
class DataArchive(Base):
    ...
    request_id = Column(Integer, ForeignKey(
        "data_requests.id", ondelete='CASCADE', name='fk_data_archive_data_requests'))
