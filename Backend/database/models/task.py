...
class TaskCategory(Base):
    ...
    parent = relationship("TaskCategory", remote_side=[
...
class Task(Base):
    ...
    user_id = Column(Integer, ForeignKey(
        "users.id", ondelete='CASCADE', name='fk_task_user_id'), nullable=False)
    assignee_id = Column(Integer, ForeignKey(
        "users.id", name='fk_assigned_user_id'))
    status = Column(SQLAEnum(TaskStatus), nullable=False,
                    default=TaskStatus.TODO.value)
    priority = Column(SQLAEnum(TaskPriority), nullable=False,
                      default=TaskPriority.MEDIUM.value)
    category_id = Column(Integer, ForeignKey(
        "task_categories.id", name='fk_task_cat_id'), nullable=False)
    workflow_id = Column(Integer, ForeignKey(
        "workflows.id", name='fk_task_workflow_id'), nullable=False)
    due_date = Column(DateTime(timezone=True))
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), onupdate=func.now())
    completed_at = Column(DateTime(timezone=True))
    creator_id = Column(Integer, ForeignKey(
        "users.id", ondelete='CASCADE', name='fk_task_creator_user_id'), nullable=False)
    ...
    def cache_task_details(self, session):
        ...
        cache_entry = CacheEntry(
            ...
            # Assuming you have a method to convert task to dict
            cache_value=str(self.id),
...
class TaskComment(Base):
    ...
    task_id = Column(Integer, ForeignKey(
        "tasks.id", name='fk_task_comm_task_id'), nullable=False)
    user_id = Column(Integer, ForeignKey(
        "users.id", name='fk_task_comm_user_id'), nullable=False)
    ...
    parent = relationship("TaskComment", remote_side=[
...
class TaskAttachment(Base):
    ...
    id = Column(Integer, primary_key=True)
    task_id = Column(Integer, ForeignKey(
        "tasks.id", name='fk_task_attach_task_id'), nullable=False)
    file_name = Column(String(255), nullable=False)
    file_path = Column(String(512), nullable=False)
    file_type = Column(String(100))
    file_size = Column(Integer)  # Size in bytes
    uploaded_by = Column(Integer, ForeignKey(
        "users.id", name='fk_task_attach_user_id'), nullable=False)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), onupdate=func.now())
...
class TaskHistory(Base):
    ...
    task_id = Column(Integer, ForeignKey(
        "tasks.id", ondelete="CASCADE", name='fk_task_hist_task_id'), nullable=False)
    user_id = Column(Integer, ForeignKey(
        "users.id", ondelete="CASCADE", name='fk_task_hist_user_id'), nullable=False)
